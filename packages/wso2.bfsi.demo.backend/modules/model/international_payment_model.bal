...
public type InternationalPaymentInitiation record {|
    ...
    string Purpose?;
    # Specifies the purpose of an international payment, when there is no corresponding 4 character code available in 
    # the ISO20022 list of Purpose Codes.
    ...
    # particular geopolitical interest, on the basis of country names obtained from the United Nations 
...
public type ExchangeRateInformation record {|
    ...
    # Specified date and time the exchange rate agreement will expire.All dates in the JSON payloads are represented 
    # in ISO 8601 date-time format. All date-time fields in responses must include the timezone. An example is below:
...
public type InternationalPaymentResponseData record {|
    ...
    # the debtor account to a creditor for a single international payment.
    InternationalPaymentInitiation Initiation;
